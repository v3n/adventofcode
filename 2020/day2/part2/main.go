...
func main() {
	...
	policies := make([]*Policy, len(fragments))

	for i, v := range fragments {
		policy := &Policy{}

		pw := strings.Split(v, ": ")
		policy.Password = pw[1]

		rn := strings.Split(pw[0], " ")
		policy.Rune = rune(rn[1][0])

		minmax := strings.Split(rn[0], "-")
		policy.Min, _ = strconv.ParseInt(minmax[0], 10, 32)
		policy.Max, _ = strconv.ParseInt(minmax[1], 10, 32)
	...
	for _, policy := range policies {
		r1, r2 := policy.Password[policy.Min-1], policy.Password[policy.Max-1]

		// boolean xor equivalent, since go does not support bitwise operations
		// on bool
		if (rune(r1) == policy.Rune) != (rune(r2) == policy.Rune) {
			total++
		}
	}
